...
import { createPanel } from "../lib/temu/panel"
...
import { getImportFilter, setProgress, setStatus } from "../lib/temu/panel"
...
export default defineContentScript({
  ...
  main() {
    window.addEventListener("message", (event) => {
      ...
      if (event.data?.type !== TEMU_REVIEWS_MESSAGE_TYPE) return
      if (!state.collecting) return

      ingestPayload(event.data.payload, getImportFilter(), (current, total, status) => {
